...
import './recruitInforPublish.scss'
import {userInfo} from "../sharedDataModule"
import defaultImg from "../../img/defaultImg.jpg"

export default function RecruitInforPublish(){
    const [job, setJob]=useState("");
    const [salary, setSalary]=useState("");
    const [numOfRecruit, setNumOfRecruit]=useState("");
    const [eduRequired, setEduRequired]=useState("请选择学历要求");
    const [workExp, setWorkExp]=useState("请选择经验要求");
    const [jobType, setJobType]=useState("请选择工作性质");
    const [ageRequired, setAgeRequired]=useState("");
    const [gender, setGender]=useState("不限");

    const [jobAdvan, setJobAdvan]=useState(""); //jobAdvan最终作废，因为setJobAdvan遇到某些技术原因无法实现目的
    const [jobIntro,setjobIntro]=useState("");
    const [promotePhoto, setPromotePhoto]=useState(defaultImg);
    ...
    function publishJob(){
        ...
        if(job==="")
            ...
            flag=false;
        else if(eduRequired==="请选择学历要求")
            flag=false;
        else if(workExp==="请选择经验要求")
            flag=false;
        ...
        if(!flag) //信息未填写完整
            ...
            const p=new Promise((resolve,reject)=>{
                Taro.request({
                    url:"http://127.0.0.1:8008/publishJob",
                    method:"GET",
                    data:{
                        job:job,
                        salary:salary,
    ...
    return (
        <View className="wrapper">
            ...
            <View className="line">
                ...
                <Input type="text" className="inputbox" placeholder="请输入职位名称" onInput={input=>{setJob(input.detail.value)}}/>
            </View>

            <View className="line">
                <Label className="label">薪资待遇</Label>
                <Input type="text" className="inputbox" placeholder="请输入薪资待遇" onInput={input=>{setSalary(input.detail.value)}}/>
            </View>

            <View className="line">
                <Label className="label">招聘人数</Label>
                <Input type="text" className="inputbox" placeholder="请输入招聘人数" onInput={input=>{setNumOfRecruit(input.detail.value)}}/>
            </View>
            ...
            <View className="line">
                ...
                <Input type="text" className="inputbox" placeholder="请输入年龄要求" onInput={input=>{setAgeRequired(input.detail.value)}}/>
            </View>
            ...
            <View className="textarea">
                <Textarea className="description" maxlength={200} placeholder="请填写职位描述, 两百字以内" onInput={input=>{setjobIntro(input.detail.value)}}></Textarea>
            </View>
            ...
            <View className="register">
                <Button className="registerbutton" style="width:100%" onClick={publishJob}>发布职位</Button>
            </View>
        </View>
    )
}